...
這也是玩家模擬工具（`game/sim/`）能可重現的基礎：注入 `mulberry32(seed)`，同 seed + 同策略 = 同結果。

### D-011 — UI 文案中英對照，集中於 `ui/strings.ts` 🟡（觀感取捨，可由設計者推翻）
GDD §1 視覺定位是 1970 年代英文點陣 NASA 控制台。**保留英文標籤、加中文小字**，
不做純中文（純中文殺美學、且綁死素材組員重做中文像素字體）。所有字串集中 `ui/strings.ts` 單一字典，
`game/` 不准知道文案存在（延伸鐵律「邏輯與算繪解耦」）。完整規格見 [`I18N_PLAN.md`](./I18N_PLAN.md)。

> **開放項**：D-011 中英對照 vs 純中文，標 🟡 待設計者最終拍板（其餘 D-005/006/008 已於 2026-06-13 鎖定）。

### D-012 — 導航改為「間歇燃燒視窗」，取代連續推進 🟡（設計者已拍板方向，數值待 sim 鎖定）
**理由**：移除 navComp 過熱後，勝利線是「導航全程開」，`nav.ts` 永不走漂移分支 → `state.rng` 與
`DEV_DRIFT_MAX_COLD` 在高手身上**完全失效**（sim 實證 rotate 五種子結局一模一樣）。改成間歇燃燒視窗：
- ETA 只在視窗內、`navComp.on && dev<=容差 && burnInput>0` 時以高推力 `COMPRESSION_BURN` 推進；視窗外滑行凍結。
- 視窗之間關導航 → 漂移分支復活 → 種子與低溫重新有牙（修掉 RNG 空轉）。
- 對應歷史 PC+2 手動點火（間歇 × 對準 × 點火）。
- **升級接縫**：核心吃 `burnInput ∈ [0,1]`，第 1 級長按給 {0,1}、第 2 級準星小遊戲給類比，`nav.ts` 不變。
- **鐵律**：勝負押在「視窗時機 + 對準 + 按住」的狀態邏輯，**不押類比手眼**，保住 sim 探針可測性。

完整契約見 [`SPEC_BURN_WINDOW.md`](./SPEC_BURN_WINDOW.md)。任務 P-burn（T-090~T-094）。
標 🟡：方向已定，`COMPRESSION_BURN` 等數值待實作後用 sim 收斂再鎖 🔒。
