...
## 2026-06-13 — 規劃輪（Opus）：方案 A 導航燃燒視窗（D-012，待別的 agent 開工）

**做了什麼**：這輪**只立規格不寫 code**。產出方案 A 的完整契約，供下一個模型照著實作 P-burn。
- 新檔 `docs/SPEC_BURN_WINDOW.md`：機制、常數、state/input/nav 改法、sim 接線、驗收、風險，逐項寫死。
- `SPEC_DECISIONS.md` 加 **D-012**（🟡 方向已拍板，數值待 sim 鎖）。
- `TASKS.md` 開 **P-burn（T-090~T-094）** 任務卡（含依賴與驗收）。

**為什麼要做這個**：上一筆修掉 navComp 過熱後，勝利線是「導航全程開」→ `nav.ts` 永不漂移
→ `state.rng` 與低溫懲罰 `DEV_DRIFT_MAX_COLD` 在高手身上**完全空轉**（rotate 五種子結局全等）。
方案 A 把導航改成**間歇燃燒視窗**：視窗內對準+按住才以高推力推進 ETA，視窗外滑行凍結、關導航→漂移復活。
一手修掉 RNG 空轉，又對應歷史 PC+2 手動點火。

**關鍵決定（給實作者）**：
- **升級接縫**：核心吃 `burnInput ∈ [0,1]`，鏡像現有 `o2Held`。第 1 級長按給 {0,1}，
  第 2 級準星小遊戲給類比，`nav.ts` 不變——升級只換 UI。
- **鐵律**：勝負押「視窗時機+對準+按住」狀態邏輯，**不押類比手眼**，否則 sim 探針失效。
- 溫度 3.2°C 命懸一線**不動**（歷史真實，留作核心張力）。

**踩到的坑 / 注意**：
- 最大風險寫在 SPEC §8：若「整局開著導航」仍可行可贏，RNG 還是死的。設計靠「視窗外 ETA 凍結+對準很快」
  把玩家推向「視窗間關導航」，**但要 sim 證實**；不夠張力再加 `AMP.burn`（點火頂安培）。先別堆機制（YAGNI）。
- `COMPRESSION_BURN ≈ 2200` 是**猜測起點**，實作後必須用 sim 收斂到「可贏、容錯 15–25%、亂玩死」再鎖。

**下一輪該知道**：P-burn 可獨立開工（依賴鏈在 TASKS）。做完要把 rotate 的 must-win 斷言移到新 `burn` 策略，
並驗證「五種子結局不再全等」＝ RNG 真的復活了。最終數值回報本日誌。

---

## 2026-06-13 — 平衡修正：navComp 不再過熱（遊戲從「不可贏」翻成「用心玩能贏」）

**做了什麼**：sim 報表顯示 25 局 0 勝，連最強的 rotate 都贏不了。深挖後確認**勝利數學上不可達**，並修正。

**根因（比上一輪日誌的「AMP 三角」更深一層）**：
- 勝利條件純算術：`ETA_INIT / COMPRESSION = 313200 / 652.5 = 480`，需要「淨 480 導航秒」，
  而這 480s 同時是 CO2 死線（`400 + 20×480 = 10000`）→ 容錯 = 0。
- 但 `navComp` 自己也會過熱（開 18s 鎖 5s，散熱 1.5/s）→ 導航工作週期上限只有 ~57%，
  最多 ~273 導航秒；再加漂移債（D-002）吃掉 ~83 等效秒 → 淨進度 ~258/480。
- 實證：nav-only 跑完整局 ETA 仍停在 52.5h（從 87h），**差近 3 倍，不是刀緣**。

**修正**：`OVERHEAT_LIMIT` 移除 navComp（`{ heater: 12, co2Filter: 8 }`）。
- 設計原則（設計師拍板）：**設備共通本質是「耗電 AMP」，「會過熱」只是發熱設備的物理特性**。
  電熱器會過熱、濾毒風扇馬達會過熱；導航電腦是固態低功耗，不過熱。導航的限制器只剩 AMP 紅線 + 漂移債。
- 重構為資料驅動：`OVERHEAT_LIMIT` 表 = 「誰會過熱」唯一真相，`devices.ts` 用 `OVERHEATING_IDS`
  （表的 keys）跑 heat 邏輯，跳電仍關全部設備（`DEVICE_IDS`）。`strategies.ts` 散熱判斷也從表推導。

**結果**：rotate 全 5 種子 WIN，eta 歸零，收在 PWR 41 / O2 17.6 / **temp 3.2°C**（新的命懸一線是溫度：
nav(4A)+co2(5A)=9A 容得下，但加暖氣 12A 爆線 → 賭 temp→0 跟 eta→0 誰先到）。nav-only 仍輸 co2（純刷導航不該贏）、
do-nothing/co2-only/panic 仍輸（亂玩會死）。`tsc` + `npx vitest run` 74/74 全綠。

**關鍵決定**：B-2 斷言從「寬鬆探針（endSecond≥300）」收緊為 **must-win**（五種子全 WIN + eta=0），
把「用心玩能贏」釘進 CI，防止未來改動又讓遊戲變不可贏。`devices.test.ts` 的 navComp 測試翻成「不過熱」。

**踩到的坑 / 注意**：
- 移除 navComp 過熱後，**`DEV_DRIFT_MAX_COLD`（低溫加重漂移）在導航全開的勝利線裡形同空轉**
  —— 導航 100% 開就永不抽樣漂移。若想讓「船艙變冷 → 導航更難」這條機制有牙，需另想設計（回報設計師）。
- 這次動了 GDD §6-A / D-005 數值（navComp 過熱上限），constants.ts 已加註說明原由。

**下一輪該知道**：平衡已可贏但溫度邊際薄（3.2°C）；若真人測覺得太緊可微調 `TEMP_NATURAL` 或 `AMP`。
