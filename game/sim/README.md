...
**⚠ 平衡洞察（設計師必看）**：`rotate` 策略死於溫度約 500s。
navComp(4A)+co2Filter(5A)=9A 時暖氣(3A)無法插入 10A 紅線。
設計師需決定是否調整 AMP 或其他數值後才能收緊 rotate 的「must win」斷言。
詳見 `../../docs/PROGRESS_LOG.md` 2026-06-13 條目。
